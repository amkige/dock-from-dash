...
const BottomDock = GObject.registerClass(
  ...
  class BottomDock extends Clutter.Actor {
    _init(layoutManager, monitor, x, y) {
      ...
      this._pressure_barrier.connectObject(
        "trigger",
        this._toggle_dock.bind(this),
        this
      );
...
const Dock = GObject.registerClass(
  class Dock extends Dash.Dash {
    ...
    _queueRedisplay() {
      if (this._workId) Main.queueDeferredWork(this._workId);
    }
    ...
    _hide_dock() {
      if (this._dock_animated || !this.work_area || this._dragging) return;
    ...
    _toggle_dock() {
      ...
      if (this.is_visible()) this._hide_dock();
      else this._show_dock();
    }

    _dock_refresh() {
      if (this._dock_refreshing) return;
      this._dock_refreshing = true;

      this.work_area = Main.layoutManager.getWorkAreaForMonitor(
        Main.layoutManager.primaryIndex
      );
      if (!this.work_area) return;
      ...
      this.show();
      if (!this._dashContainer.get_hover()) this._hide_dock();
...
export default class DockFromDashExtension {
  ...
  _update_hot_edges() {
    for (let i = 0; i < Main.layoutManager.monitors.length; i++) {
      ...
      } else Main.layoutManager.hotCorners.push(null);
    }
  }
  ...
  enable() {
    ...
    Main.layoutManager.connectObject(
      "startup-complete",
      () => {
        if (!SHOW_OVERVIEW_AT_STARTUP) Main.overview.hide();
      },
      this
    );
  }
